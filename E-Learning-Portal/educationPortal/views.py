...
import json
from .models import Assignment, Comment, Conversation, Material, FileModel, MCanswer, MultipleChoiceQuestion, Quiz, QuizSubmission, Submission, User, Classroom, Announcement, Text
...
def material(request, code):
    ...
    return render(request, "educationPortal/material.html", {
        "class": classroom,
        "date":  datetime.now(),
        "material": materials
    })
...
def createMaterial(request, code):
    ...
    if request.method == "POST":
        ...
        material.description = request.POST["instructions"]
        material.classroom = classroom
        #material.duedate = request.POST["assignmentDueDate"]
        material.save()
...
def viewMaterial(request, code, id):
    classroom = Classroom.objects.get(code=code)
    material = Material.objects.get(id=id)

    submission = None
    if(material.submissions.filter(user=request.user).exists()):  # for student
        submission = material.submissions.get(user=request.user)
    ...
    return render(request, "educationPortal/viewMaterial.html", {
        "class": classroom,
        "material": material,
        "submission": submission  # one student submission (for student)
